...
export default async function handler(
  ...
  if (!session && process.env.NODE_ENV !== "test")
    return res.writeHead(403, statusMessages[403]).end();
      ...
      try {
        ...
        if (fields.product) {
          ...
          if (files["product-main-img"]) {
            ...
            const img = (await uploadImagesToCloudinary(
              files["product-main-img"][0].filepath
            )) as string;
        ...
        if (e instanceof Prisma.PrismaClientValidationError) {
          ...
          return res
            .writeHead(400, statusMessages[400], {
              "Content-Type": "text/plain",
            })
            .end(
              "Some required fields are missing. Please check the data sent and try again"
            );
